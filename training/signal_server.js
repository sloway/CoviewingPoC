...
var http_server = http.createServer(function (request, response) {
  var matches = request.url.match("^/images/(.*)");
  if (matches) {
...
fs.readFile("./videochat.html", function (error, data) {
  if (error) {
    log_error(error);
  } else {
    page = data;
        ...
        if (signal.type === "join" && signal.token !== undefined) {
          ...
          try {
            Object.keys(webrtc_discussions[signal.token]).forEach(function (
              ...
              if (id != connection.id) {
                webrtc_clients[id].send(message.utf8Data, log_error, log_error);
              }
            });
          } catch (e) {
            log_comment("Exception : " + e);
